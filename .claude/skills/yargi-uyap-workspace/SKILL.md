...
```bash
npx dava-cli@latest clone [--dir <path>] [--target antigravity,claude] [--host avukat|avukatbeta] [--timeout <s>]
```
...
antigravity; virgül listesi olabilir — **Claude Code için `--target claude`**).
...
```bash
npx dava-cli@latest init [--dir <path>] [--target antigravity,claude]
```
...
- `AGENTS.md` / `CLAUDE.md` — sürekli proje talimatları + `.agents/`/`.claude/`
...
- `clone` tüm projeyi kendisi kurar — önce `init` ÇALIŞTIRMA. `init` yalnız
  UYAP-dışı özel klasör içindir. Claude Code desteği için `--target claude`.
